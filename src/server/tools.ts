...
export function registerTools(server: McpServer): void {
  ...
  // ---------------------------------------------------------------------------
  // ai-summarize — SAMPLING: the server asks the *client's LLM* for a
  // completion (sampling/createMessage).
  // ---------------------------------------------------------------------------
  server.registerTool(
    'ai-summarize',
    {
      title: 'AI Summarize',
      description:
        'Summarizes text by sampling the client LLM (sampling/createMessage). The server does no AI itself.',
      inputSchema: {
    ...
    async ({ text }) => {
      try {
        const result = await server.server.createMessage({
          systemPrompt: 'You are a concise summarizer. Answer in one short sentence.',
          messages: [
            {
              role: 'user',
          ...
          maxTokens: 200,
          modelPreferences: { hints: [{ name: 'fake-demo-model' }] },
        })
        const summary = result.content.type === 'text' ? result.content.text : '[non-text content]'
        return {
          content: [
            {
              type: 'text',
              text: `Sampling result from model "${result.model}":\n${summary}`,
            },
          ],
        }
      } catch (err) {
        return {
          ...
          content: [
            {
              type: 'text',
              text: `Sampling failed (the client may not support it): ${errorMessage(err)}`,
            },
          ],
        }
      }
  ...
  // ---------------------------------------------------------------------------
  // explore-roots — ROOTS: the server asks the client which filesystem roots
  // it exposes (roots/list).
  // ---------------------------------------------------------------------------
  server.registerTool(
    'explore-roots',
    {
      title: 'Explore Client Roots',
      description: 'Lists the filesystem roots exposed by the client (roots/list).',
      annotations: { readOnlyHint: true, openWorldHint: false },
    },
    async () => {
      try {
        ...
        const text = roots.length
          ? `Client roots:\n${roots.map((r) => `- ${r.name ?? '(unnamed)'} — ${r.uri}`).join('\n')}`
          : 'The client did not expose any roots.'
        ...
        return {
          ...
          content: [
            {
              type: 'text',
              text: `Listing roots failed (the client may not support it): ${errorMessage(err)}`,
            },
          ],
        }
      }
